...
use super::expr::parse_expr_until_either_with_asi;
use super::expr::Asi;
...
use super::signature::parse_signature_function;
...
pub fn parse_class_or_object_member(
  ...
  let key = if parser.consume_if(TokenType::BracketOpen)?.is_match() {
    let key =
      ClassOrObjectMemberKey::Computed(parse_expr(scope, parser, TokenType::BracketClose, syntax)?);
  ...
  let value = if is_generator || is_async || parser.peek()?.typ() == TokenType::ParenthesisOpen {
    ...
    ClassOrObjectMemberValue::Method {
      ...
      signature,
      body: parse_stmt_block(scope, parser, &ParsePatternSyntax {
