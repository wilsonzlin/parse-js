use super::class_or_object::parse_class_body;
use super::class_or_object::parse_class_or_object_member;
...
pub fn parse_jsx_name(scope: ScopeId, parser: &mut Parser) -> SyntaxResult<NodeId> {
  ...
  Ok(if parser.consume_if(TokenType::Colon)?.is_match() {
    ...
    parser.create_node(scope, start.loc() + name.loc(), Syntax::JsxName {
      namespace: Some(start.loc().clone()),
      name: name.loc().clone(),
    })
  } else {
    // Plain name.
    parser.create_node(scope, start.loc().clone(), Syntax::JsxName {
      namespace: None,
      ...
      .match_loc()
    {
      ...
      Some(start) => Some({
        if parser.consume_if(TokenType::Colon)?.is_match() {
          // Namespaced name.
          let name = parser.require_with_mode(TokenType::Identifier, LexMode::JsxTag)?;
          parser.create_node(scope, start + name.loc(), Syntax::JsxName {
            namespace: Some(start.clone()),
            name: name.loc().clone(),
          })
        } else if parser.peek()?.typ() == TokenType::Dot && !start.as_slice().contains(&b'-') {
          // Member name.
          let mut path = vec![];
          while parser.consume_if(TokenType::Dot)?.is_match() {
            path.push(parser.require(TokenType::Identifier)?.loc().clone());
          }
          parser.create_node(scope, start.add_option(path.last()), Syntax::JsxMember {
            base: start.clone(),
            path,
          })
        } else {
          // Plain name.
          parser.create_node(scope, start.clone(), Syntax::JsxName {
            namespace: None,
            name: start.clone(),
          })
        }
...
pub fn parse_jsx_element(
    ...
    loop {
      ...
      let value = if !parser.consume_if(TokenType::Equals)?.is_match() {
        ...
        Some(if parser.consume_if(TokenType::BraceOpen)?.is_match() {
          ...
          parser.create_node(scope, value.loc().clone(), Syntax::JsxText {
  ...
  Ok(if parser.consume_if(TokenType::Slash)?.is_match() {
    ...
    parser.create_node(scope, tag_start.loc() + end.loc(), Syntax::JsxElement {
      name: tag_name,
      attributes,
      children: vec![],
    })
  } else {
    parser.require(TokenType::ChevronRight)?;
    ...
    let close_start = loop {
      ...
      let text = parser.require_with_mode(TokenType::JsxTextContent, LexMode::JsxTextContent)?;
      if !text.loc().is_empty() {
        children.push(
          parser.create_node(scope, text.loc().clone(), Syntax::JsxText {
            value: text.loc().clone(),
          }),
        );
      };
      if parser.peek()?.typ() == TokenType::ChevronLeft {
        children.push(parse_jsx_element(scope, parser, syntax)?);
      };
      if parser.consume_if(TokenType::BraceOpen)?.is_match() {
        // TODO Allow empty expr.
        let value = parse_expr(scope, parser, TokenType::BraceClose, syntax)?;
        children.push(parser.create_node(
          scope,
          parser[value].loc().clone(),
          Syntax::JsxExpressionContainer { value },
        ));
...
pub fn parse_call_args(
  ...
  loop {
    ...
    args.push(
      parser.create_node(scope, parser[value].loc().clone(), Syntax::CallArg {
        spread,
        value,
      }),
    );
...
pub fn parse_expr_array(
  ...
  Ok(
    parser.create_node(scope, &loc_start + &loc_end, Syntax::LiteralArrayExpr {
      elements,
    }),
  )
}
...
pub fn parse_expr_object(
  ...
  Ok(
    parser.create_node(scope, &loc_start + &loc_end, Syntax::LiteralObjectExpr {
      members,
    }),
  )
}
...
pub fn parse_expr_arrow_function(
  ...
  let (signature, arrow) = if !is_async
    && is_valid_pattern_identifier(parser.peek()?.typ(), &ParsePatternSyntax {
      await_allowed: false,
    ...
    let pattern = parser.create_node(fn_scope, param_name.clone(), Syntax::IdentifierPattern {
      name: param_name.clone(),
    });
    parser[fn_scope].add_block_symbol(param_name.clone(), pattern)?;
    let param = parser.create_node(scope, param_name.clone(), Syntax::ParamDecl {
      rest: false,
      pattern,
      default_value: None,
    });
    let signature = parser.create_node(scope, param_name.clone(), Syntax::FunctionSignature {
...
pub fn parse_expr_import(
  ...
  Ok(
    parser.create_node(scope, start.loc() + end.loc(), Syntax::ImportExpr {
      module,
    }),
  )
}
...
pub fn parse_expr_function(
  ...
  let name = match parser.peek()? {
    t if is_valid_pattern_identifier(t.typ(), syntax) => {
      ...
      let name_node = parser.create_node(fn_scope, t.loc().clone(), Syntax::ClassOrFunctionName {
        name: t.loc().clone(),
      });
  ...
  let signature = parse_signature_function(fn_scope, parser, syntax)?;
  let body = parse_stmt_block(fn_scope, parser, &ParsePatternSyntax {
    await_allowed: !is_async && syntax.await_allowed,
    yield_allowed: !generator && syntax.yield_allowed,
  })?;
  Ok(
    parser.create_node(scope, &start + parser[body].loc(), Syntax::FunctionExpr {
      parenthesised: false,
      is_async,
      generator,
      name,
...
pub fn parse_expr_class(
  ...
  let name = match parser.peek()? {
    t if is_valid_pattern_identifier(t.typ(), syntax) => {
      parser.consume_peeked();
      let name_node = parser.create_node(scope, t.loc().clone(), Syntax::ClassOrFunctionName {
        name: t.loc().clone(),
      });
  ...
  Ok(parser.create_node(scope, &start + &end, Syntax::ClassExpr {
    parenthesised: false,
    name,
    extends,
    members,
    ...
    Some(operator)
      if (
        ...
        (operator.name != OperatorName::Await && operator.name != OperatorName::Yield)
          || (operator.name == OperatorName::Await && !syntax.await_allowed)
          || (operator.name == OperatorName::Yield && !syntax.yield_allowed)
      ) =>
    {
    ...
    _ => {
      match t.typ() {
        ...
        TokenType::ChevronLeft => {
          parser.restore_checkpoint(cp);
          parse_jsx_element(scope, parser, syntax)?
        }
        ...
        TokenType::LiteralTrue | TokenType::LiteralFalse => {
          parser.create_node(scope, t.loc().clone(), Syntax::LiteralBooleanExpr {
        ...
        TokenType::LiteralNumber => {
          parser.create_node(scope, t.loc().clone(), Syntax::LiteralNumberExpr {
        ...
        TokenType::LiteralString => {
          parser.create_node(scope, t.loc().clone(), Syntax::LiteralStringExpr {
        ...
        TokenType::LiteralTemplatePartStringEnd => {
          ...
          parser.create_node(scope, t.loc().clone(), Syntax::LiteralTemplateExpr {
            parts,
          })
        }
...
fn transform_literal_expr_to_destructuring_pattern(
  ...
  match parser[node].stx() {
    Syntax::LiteralArrayExpr { elements } => {
      ...
      Ok(
        updates.create_node(scope, loc.clone(), Syntax::ArrayPattern {
    ...
    Syntax::LiteralObjectExpr { members } => {
          ...
          Syntax::ObjectMember { typ } => match typ {
            ObjectMemberType::Valued { key, value } => {
              let (target, default_value) = match value {
                ...
                _ => return Err(parser[node].error(SyntaxErrorType::InvalidAssigmentTarget)),
              };
      ...
      Ok(
        updates.create_node(scope, loc.clone(), Syntax::ObjectPattern {
          properties,
          rest,
        }),
      )
    ...
    Syntax::IdentifierExpr { name } | Syntax::IdentifierPattern { name } => Ok(
      updates.create_node(scope, loc.clone(), Syntax::IdentifierPattern {
        name: name.clone(),
      }),
    ),
...
fn convert_assignment_lhs_to_target(
  ...
  match parser[lhs].stx() {
    ...
    | Syntax::IdentifierExpr { .. }) => {
      ...
      let root = transform_literal_expr_to_destructuring_pattern(scope, parser, lhs, &mut updates)?;
      ...
      Some(operator) => {
        ...
        left = match operator.name {
          ...
          OperatorName::MemberAccess | OperatorName::OptionalChainingMemberAccess => {
            ...
            parser.create_node(scope, parser[left].loc() + &right, Syntax::MemberExpr {
              parenthesised: false,
              optional_chaining: match operator.name {
