use super::decl::parse_decl_class;
use super::decl::VarDeclParseMode;
use super::expr::parse_expr_with_asi;
use super::expr::Asi;
...
use crate::parse::decl::parse_decl_function;
use crate::parse::decl::parse_decl_var;
...
fn parse_import_or_export_name(
  ...
  let alias_node = parser.create_node(scope, alias.clone(), Syntax::IdentifierPattern {
    name: alias.clone(),
  });
  if add_to_scope {
...
pub fn parse_stmt_var(
  ...
  Ok(
    parser.create_node(scope, parser[declaration].loc().clone(), Syntax::VarStmt {
      declaration,
    }),
  )
}
...
pub fn parse_stmt_export(
  ...
  Ok(match t.typ() {
    TokenType::BraceOpen => {
      ...
      // TODO Loc
      parser.create_node(scope, start.loc().clone(), Syntax::ExportListStmt {
...
pub fn parse_stmt_for(
  ...
  let lhs_raw = match parser.peek()?.typ() {
    TokenType::KeywordVar | TokenType::KeywordLet | TokenType::KeywordConst => LhsRaw::Declaration(
      parse_decl_var(for_scope, parser, VarDeclParseMode::Leftmost, syntax)?,
    ),
    ...
    _ => {
      ...
      let checkpoint = parser.checkpoint();
      match if let Ok(node) = parse_pattern(for_scope, parser, ParsePatternAction::None, syntax) {
        match parser.peek()?.typ() {
  ...
  let header = match parser.peek()?.typ() {
    ...
    _ => {
      ...
      let init = match lhs_raw {
        ...
        LhsRaw::Pattern(_) => return Err(start.error(SyntaxErrorType::ForLoopHeaderHasInvalidLhs)),
      };
  ...
  Ok(
    parser.create_node(scope, start.loc() + parser[body].loc(), Syntax::ForStmt {
      header,
      body,
    }),
  )
...
pub fn parse_stmt_if(
  ...
  Ok(
    parser.create_node(scope, start.loc() + parser[*end].loc(), Syntax::IfStmt {
      test,
      consequent,
      alternate,
    }),
...
pub fn parse_stmt_import_or_expr_import(
  ...
  // TODO Loc
  Ok(
    parser.create_node(scope, start.loc().clone(), Syntax::ImportStmt {
      default,
      module,
      names,
    }),
...
pub fn parse_stmt_try(
  ...
  let catch = if parser.consume_if(TokenType::KeywordCatch)?.is_match() {
    ...
    Some(
      parser.create_node(scope, parser[body].loc().clone(), Syntax::CatchBlock {
        parameter,
        body,
      }),
    )
...
pub fn parse_stmt_while(
  ...
  Ok(
    parser.create_node(scope, start.loc() + parser[body].loc(), Syntax::WhileStmt {
      condition,
      body,
    }),
  )
...
pub fn parse_stmt_do_while(
  ...
  Ok(
    parser.create_node(scope, start.loc() + end.loc(), Syntax::DoWhileStmt {
      condition,
      body,
    }),
  )
...
pub fn parse_stmt_switch(
  ...
  Ok(
    parser.create_node(scope, start.loc() + end.loc(), Syntax::SwitchStmt {
      test,
      branches,
    }),
  )
